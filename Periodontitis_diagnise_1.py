...
# Remove all empty rows values from the dataset including the diagnosis label's.
#use linear correlation heatmap to see the correlation of each variable with the diagnosis label.
#remove the columns that are not well correlated. 
#remove BMI value row of 93
# no need to write train accuracy.
#use PCA on the the reduced columns dataset.
#use 3 algorithms for prediction.
...
def data_preprocess():
    ...
    data1 = pd.read_excel("/home/umair/software-projects/python/Computational-Intelligence-Project/data1.xlsx")
    data2 = data1.copy();
    ...
    print("shape of data to be used")
    print(data1.shape)
    return data1
...
def data_select(i):
    data = data_preprocess()
    print(data.iloc[1,[2,3,4,5,6,7,8,9]])
    if i == 0:
    ...
    if i == 3:
        x_data = data.iloc[:, [3,8,14]]
    y_data = data.loc[:, ['Diagnosis']]
    return x_data, y_data

def data_select_label(i):
    data = data_preprocess()
    print("label used")
...
#depth 14: labels:all, acc:0.78
#8, all, 0.78
#3,all, 0.78, train: 0.875
